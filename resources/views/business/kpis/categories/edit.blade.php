        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('business.kpis.categories.update', $category) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="image" :value="__('KPI Image (Optional)')" />
                            @if($category->image_path)
                                <div class="mt-2 mb-3">
                                    <img src="{{ Storage::url($category->image_path) }}" alt="{{ $category->name }}" class="h-32 w-32 object-cover rounded-lg border border-gray-200">
                                </div>
                            @endif
                        ...
                        <div>
                            <x-input-label for="points_reward" :value="__('Points Reward per Achievement')" />
                        ...
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('business.kpis.categories.index') }}" class="text-sm text-gray-600 hover:text-gray-900">Cancel</a>
                            <x-primary-button>
                                {{ __('Update KPI Category') }}
                            </x-primary-button>
                        </div>
                    </form>
                </div>
