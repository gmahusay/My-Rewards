            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('business.customers.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500 focus:bg-indigo-500 active:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Add New Customer
                        </a>
                    </div>
                                        ...
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($customer->profile_photo_path)
                                                    <img src="{{ Storage::url($customer->profile_photo_path) }}" alt="{{ $customer->name }}" class="h-10 w-10 rounded-full object-cover">
                                                @else
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($customer->points) }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $customer->created_at->format('M d, Y') }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <div class="flex items-center space-x-3">
                                                    <button @click="showPointsModal = true; selectedUser = '{{ $customer->name }}'; userId = {{ $customer->id }}" class="text-indigo-600 hover:text-indigo-900 border border-indigo-600 rounded px-2 py-1 text-xs">Allocator Points</button>
                                                    <a href="{{ route('business.customers.edit', $customer->id) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                                    <form action="{{ route('business.customers.destroy', $customer->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this customer?');">
                                                        @csrf
                                                        @method('DELETE')
                                                ...
                                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                                    ...
                                                    <div class="mt-2">
                                                        <p class="text-sm text-gray-500">
                                                            Enter the amount of points to transfer to this customer.
                                                        </p>
                                                        <div class="mt-4">
