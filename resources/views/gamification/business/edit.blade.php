<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Campaign: {{ $campaign->title }}</h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8">
                    <form method="POST" action="{{ route('business.gamification.update', $campaign) }}" class="space-y-8" enctype="multipart/form-data">
                        @csrf
                        @method('PUT') id="campaign-form" class="space-y-8">
                        @csrf
                            ...
                            <div class="flex items-start gap-6">
                                <div id="logo-preview-wrapper" class="shrink-0 h-24 w-24 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex items-center justify-center overflow-hidden">
                                    @if($campaign->logo_path)
                                        <img id="logo-preview" src="{{ Storage::url($campaign->logo_path) }}" alt="Logo Preview" class="h-24 w-24 object-cover rounded-xl" />
                                        <svg id="logo-placeholder" class="h-10 w-10 text-gray-300 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">Leave blank to keep existing logo. Max 2MB (JPG, PNG, JPEG, WEBP, GIF).</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="reward_points" name="reward_points" type="number" class="mt-1 block w-full"
                                                  :value="old('reward_points', $campaign->reward_points)" min="0" required />
                                ...
                                <div>
                                    ...
                                    <select id="is_active" name="is_active"
                                        ...
                                        <option value="1" {{ old('is_active', $campaign->is_active) ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ !old('is_active', $campaign->is_active) ? 'selected' : '' }}>Inactive</option>
                                    </select>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            @php
                $existingTargetsJson = $campaign->targets->sortBy('level')->values()->map(function($t) {
                    return ['type' => $t->target_type, 'product_id' => $t->product_id, 'icon' => $t->icon, 'label' => $t->label, 'value' => $t->target_value];
                });
            @endphp
            const existingTargets = @json($existingTargetsJson);
            if (existingTargets.length > 0) {
                existingTargets.forEach(t => addTarget(t));
            } else {
                addTarget();
            }
        });
